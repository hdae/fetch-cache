...
- **There is no way to supply your own cache key** (the `cacheKey` draft never
  shipped; 0.5.0 also removed the `key` option that existed during
  development). If you used `cacheName` to _partition_ caches (per app / per
  tenant), there is no direct successor — `evict` prefixes only address the
  HF content keys. Partition instead by passing a custom `caches`
  implementation (e.g. a thin wrapper remapping `open(name)` to a per-tenant
  name) to every call — the admin APIs accept the same `{ caches }` — or keep
  the URLs themselves distinct.
