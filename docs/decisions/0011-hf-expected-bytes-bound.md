...
上限として使う以上、**形式不正の申告は `toSpec` が要求の前に弾く**（`sha256` の形式検査と
同じ語彙・同じ位置 — path 付きで報告し、可変 ref の解決 API にも出ない）。受理するのは 0 以上
の安全整数だけで、`-1` や `1.5` は throw する。どのみち実長と一致し得ない申告であり、上限に
なった今は「最初のチャンクで申告を超えた」という事由の紛れる失敗に化けるため、早い方が厳密に
良い（ADR 0007 が確保失敗について下したのと同じ判断）。
...
- **失敗の内容は変わらず、時刻だけが早くなる**。宣言を超える応答は従来も必ず失敗していた
  ...
  「どこまで受け取ってから落ちるか」だけ。**例外が 2 つあり、そこは v0.6.0 で成功していた
  呼び出しが throw に変わる**: ①温め側 spec の `expectedBytes` が実長より小さく、読み出し側
  spec が同じ宣言を持たない場合 — v0.6.0 は温めも読み出しも成功していたが、`prefetchHfFile`
  が超過で落ちるようになる（読み出し側にも同じ宣言があれば従来も self-heal 側で落ちていたので、
  新しい失敗はこの 1 通りだけ）。②形式不正（負・非整数）の `expectedBytes` — v0.6.0 の prefetch
  は spec の `expectedBytes` を見ておらず成功していたが、上限として渡す今は `toSpec` が要求の
  前に throw する（`fetchHfFile` では従来も長さ検証で落ちていた）。
- 文言は変わる: 超過は
  `fetch-cache: 受信が申告 {maxBytes} バイトを超えた（{n} バイト以上） ({url})`（`）` と
  `({url})` の間は半角空白）。`response.body` が null で arrayBuffer フォールバックへ落ちる
  経路だけは打ち切りではなく読み切り後の判定なので、「以上」が付かず `{n}` に実長が入る変種に
  なる。全量後の `バイト数不一致: {実長} != {宣言}` は**不足のときだけ**出るようになる。
...
- `response.body` が null のランタイム（両経路の arrayBuffer フォールバック）では打ち切れず、
  全量が届いてから長さで判定する — この経路で上限が変えるのは**エラーの内容だけで、転送量は
  減らない**（帯域の節約は body が stream で読めるランタイムに限る）。
