...
- **single-flight は cache 有効の GET のみ**（0.3.0 で導入 — DECIDED: docs/decisions/0004）。
  ...
  `init` / `onCacheError` / `expectedBytes` / `retry` / `onRetry` は使われない**（取得は先行
  呼び出しのオプションで走る。合流者は leader の再試行をそのまま待つ。認証ヘッダ違いを
  区別しないのはキャッシュキーがヘッダ非依存の設計と同じ割り切り）。
  ...
  いない）合流者まで落ちる（合流者側の容量不足だけがその呼び出しに留まる）。leader の
  `init.signal` による中断も同じく取得失敗として合流者全員へ伝播する（逆に合流者自身の
  `signal` は使われないので、leader の再試行の待機を打ち切ることはできない）。
...
- **`expectedBytes` / content-length は確保ヒントであって検証ではない**（DECIDED:
  ...
  ヒントのまま不変で、上限を渡す口は内部導管にしか無い）。`expectedBytes: 0` は形式不正では
  なく妥当な申告（0 バイトファイル）で、**確保ヒントとしてだけ無視される** — 確保する意味が
  無いので受信バッファは事前確保しないが、HF 層では上限 0・長さの厳密一致 0 として効く
  （1 バイトでも届けば「申告 0 バイトを超えた」で落ちる）。
- **再試行するのはステータスで見える rate limit / 一時的な不能だけ**（DECIDED:
  docs/decisions/0010）。既定の対象は 429 / 503 で、接続エラー・受信途中の切断・その他の
  4xx / 5xx は 1 回目でそのまま throw する（成功に転じる根拠が無いため）。**対象の method は
  GET / HEAD だけ** — `cache: false` なら任意の method を通せるが（ADR 0002）、冪等でない要求
  （`POST` + body 等）は 429 / 503 でも打ち直さず最初の応答で throw する。**`maxDelayMs` を
  渡さない限り `Retry-After` の指示どおり待つ** — 上限を勝手に決めないのは、長い待機ほど
  「その時間待たなければ通らない」ことを意味し、短く切れば再試行を消費するだけになるため。
  **ただし `setTimeout` の上限（`2**31 - 1` ms ≒ 24.8 日）を超える待機は再試行しない** —
  超える指定は即時発火するので待ったことにできず、指示を無視して打ち直すだけになる。返るのは
  **そのラウンドの応答**で、そのラウンドは再試行回数に数えず `onRetry` も呼ばないが、それまでに
  再試行していればその回数は残り、文言末尾の `（再試行 N 回の後）` にも出る（「最初の応答」が
  返るのは上の GET / HEAD 以外の経路だけ）。判定は上限適用後なので、`maxDelayMs` を渡せば
  切り詰めた待機で再試行できる。恒久的に 503 を返すサーバに対しては、**`Retry-After` が
  無ければ**既定で最大 31 秒（1+2+4+8+16）待ってから落ちる。`Retry-After` 付きなら上限は
  掛からず、指示どおりの待機が最大 `maxRetries` 回（既定 5）**累積する** —
  `Retry-After: 3600` を返し続けるサーバなら合計 5 時間になる。急ぐ呼び出しは `maxRetries` /
  `maxDelayMs` を絞るか `retry: false` を渡す。**`retry` の値の形式不正（`NaN`・負・非整数・
  400〜599 の外のステータス）は network に出る前に throw する** — 黙って受けると上限ガードと
  回数の打ち切りが裏返るため（検査は network 直前なので、キャッシュヒットで決着する呼び出しでは
  throw しない）。self-heal で evict した後の network 再取得も再試行を通るので、**その待機中は
  エントリが存在しない**（同じキーの他の読み出しは miss になる）。合流者（single-flight）
...
- **prefetch が見る spec は `sha256` と `expectedBytes` だけ**（DECIDED: docs/decisions/0005
  ...
  `expectedBytes` を prefetch が見るようになったのは 0.7.0 からで、**それまで成功していた
  温めが throw に変わる場合がある** — 宣言より大きい応答（読み出し側 spec が同じ宣言を持たない
  なら、v0.6.0 では温めも読み出しも成功していた）と、形式不正（負・非整数）の申告（v0.6.0 の
  prefetch は無視していたが、上限として渡す今は要求の前に throw する）の 2 通り。
