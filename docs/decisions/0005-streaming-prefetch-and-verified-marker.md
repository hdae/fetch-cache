...
- network 経路の JS ヒープが 2N → 1N（1 と 2 の合わせ技）。`prefetchUrl` を使えば ~0 まで
  落とせるが、その場合の検証は読み出し時の 1N バッファに対して行われる。
- 未検証バイトがキャッシュに載る窓が新経路にだけ生まれる。self-heal が唯一の回復手段なので、
  検証の要である `validate` を読み出し側で必ず渡すことが `prefetchUrl` 利用の前提になる。
- `prefetchUrl` で温めたエントリには印が付かないため、`verifiedMarker` の恩恵は
  「fetchBytes で取得したエントリ」に限られる。両者を組み合わせて「初回 materialize の検証後に
  印だけ書き足す」機構は、N バイトの再 put を伴うため今回は入れない（要望が出たら別途裁定）。
